...
public class Main {
    ...
    private static void processMenuOption(int option) {
        do {
            switch (option) {
                ...
                case 4:
                    System.out.println("=== Remove POI by City ===");
                    processRemovePoiByCity();
                    break;
        ...
        while (continueAdding) {
            try {
                ...
                if (POIManagment.isIDAvailable(id)) {
                    Double latitude = LibIO.solicitarDouble("Introduce la latitud: ", 2, 100);
                    Double longitude = LibIO.solicitarDouble("Introduce la longitud: ", 2, 100);
    ...
    private static void processListByIDPartial() {
        try {
            Integer partialId = LibIO.solicitarInt("Introduce parte del ID a buscar: ", 1, 100);

            List<POI> pois = (List<POI>) POIManagment.getSourceDB(true).listByID(partialId);
            pois.forEach(System.out::println);
        } catch (Exception e) {
    ...
    // Procesar la opción de eliminar POI por ciudad
    private static void processRemovePoiByCity() {
        try {
            String city = LibIO.solicitarString("Introduce la ciudad de los POIs a eliminar: ", 1, 100);

            List<POI> removedPois = POIManagment.getSourceDB(true).removeByCity(city);
            if (!removedPois.isEmpty()) {
                System.out.println("POIs eliminados:");
                removedPois.forEach(System.out::println);
            } else {
                System.out.println("No se encontraron POIs de la ciudad especificada.");
            }
        } catch (Exception e) {
